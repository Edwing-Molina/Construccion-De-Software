...
class _WorkPatternScreenState extends State<WorkPatternScreen> {
  ...
  late final WorkPatternService _workPatternService =
    WorkPatternService(AuthService());
      ...
      if (resp.success && resp.data != null) {
        setState(() {
          _workPatterns
            ..clear()
            ..addAll(resp.data!);
            _horariosGenerados = _workPatterns.isNotEmpty;
        });
      } else {
  ...
  Future<void> _generateSchedules() async {
    if (_horariosGenerados) return;
    if (_workPatterns.isEmpty) {
      _showSnackBar('Primero crea patrones de trabajo');
      return;
    }

    final startDate = _workPatterns
      .map((e) => e.startDateEffective)
      .reduce((a, b) => a.isBefore(b) ? a : b);

    final endDate = _workPatterns
      .where((e) => e.endDateEffective != null)
      .map((e) => e.endDateEffective!)
      .fold<DateTime?>(
        null,
    ...
    try {
      final resp = 
        await _workPatternService.generarHorarios(startDate, endDate);
      if (resp.success) {
        _showSnackBar('Horarios generados correctamente');
        setState(() => _horariosGenerados = true);
      } else {
        _showSnackBar(resp.message);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    final displayedPatterns = _showInactive
      ? _workPatterns
              ...
              if (displayedPatterns.isEmpty)
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context, index) =>
                    _buildPatternCard(displayedPatterns[index]),
                ),
            ],
          ),
        ),
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          TextFormField(
            decoration: _inputDecoration('Duración en días'),
          ...
          DropdownButtonFormField<int>(
            decoration: _inputDecoration('Duración de slot (minutos)'),
          ...
          SwitchListTile(
            ...
            activeColor: _doradoUady,
            contentPadding: EdgeInsets.zero,
          ),
          const SizedBox(height: 20),
          ElevatedButton(
            style: _buttonStyle(),
            onPressed: _isLoading ? null : _createWorkPattern,
  ...
  Widget _buildToggleInactiveButton() {
    return ElevatedButton(
      style: _buttonStyle(padding: const EdgeInsets.symmetric(vertical: 12)),
  ...
  Widget _buildPatternCard(DoctorWorkPattern p) {
    return Card(
      ...
      child: ListTile(
        title: Text(
          '${p.dayOfWeek.name} - '
          '${_formatTimeOfDay(p.startTimePattern)} a '
          '${_formatTimeOfDay(p.endTimePattern)}',
        ...
        subtitle: Text(
          'Desde: ${_formatDate(p.startDateEffective)} '
          'hasta: ${p.endDateEffective != null ? _formatDate(p.endDateEffective) : "Indefinido"}\n'
          'Slots: ${p.slotDurationMinutes} min\n'
          'Activo: ${p.isActive ? "Sí" : "No"}',
        ),
        trailing: p.isActive
            ? IconButton(
                icon: const Icon(Icons.block, color: _doradoUady),
                tooltip: 'Desactivar patrón',
                onPressed: () => _deactivatePattern(p.id!),
              )
            : null,
      ),
    );
